...
class ExcelExporter:
    ...
    COLUMNS = [
        ...
        ('Line Items Count', '_line_items_count'),
    ]
    
    LINE_ITEM_COLUMNS = [
        ('Source Invoice', '_source_invoice'),
        ('Row #', 'row_index'),
        ('Item Code', 'item_code'),
    ...
    def export(
        ...
        try:
            ...
            # Create line items sheet if any results have line items
            self._create_line_items_sheet(workbook, results)
    ...
    def _create_data_sheet(
        ...
        for row_num, result in enumerate(results, 2):
            for col, (_, field_name) in enumerate(self.COLUMNS, 1):
                if field_name == '_line_items_count':
                    value = getattr(result, 'line_items_count', 0)
                else:
    ...
    def _create_line_items_sheet(
        ...
        """
        Create a sheet with line item details from all invoices.
        
        Args:
            workbook: openpyxl Workbook instance.
            results: List of extraction results (may include HybridExtractionResult).
        """
        from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
        from openpyxl.utils import get_column_letter
        
        # Collect all line items across all results
        all_line_items = []
        for result in results:
            ...
            if items:
                ...
                for item in items:
                    all_line_items.append((invoice_id, item))
        
        if not all_line_items:
            logger.debug("No line items found; skipping Line Items sheet.")
            return
        
        sheet = workbook.create_sheet(title="Line Items")
        ...
        # Write headers
        for col, (header_name, _) in enumerate(self.LINE_ITEM_COLUMNS, 1):
            cell = sheet.cell(row=1, column=col, value=header_name)
            cell.font = header_font
            cell.fill = header_fill
            cell.alignment = header_alignment
        ...
        # Write data rows
        for row_num, (invoice_id, item) in enumerate(all_line_items, 2):
            for col, (_, field_name) in enumerate(self.LINE_ITEM_COLUMNS, 1):
                if field_name == '_source_invoice':
                    value = invoice_id
                elif field_name in ('unit_price', 'total', 'tax_amount', 'discount'):
        ...
        for col, (header_name, _) in enumerate(self.LINE_ITEM_COLUMNS, 1):
            column_letter = get_column_letter(col)
            max_length = len(header_name)
            for row in range(2, len(all_line_items) + 2):
                cell_value = sheet.cell(row=row, column=col).value
                if cell_value:
                    max_length = max(max_length, len(str(cell_value)))
            sheet.column_dimensions[column_letter].width = min(max_length + 2, 50)
        ...
        logger.info(f"Line Items sheet created with {len(all_line_items)} items")
