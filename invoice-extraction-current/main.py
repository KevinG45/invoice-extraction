...
def run_extraction(
    ...
    from src.model_inference import HybridExtractor
    ...
    extractor = HybridExtractor(
        use_layoutlm=config.get("model.hybrid.use_layoutlm", True),
        use_donut=config.get("model.hybrid.use_donut", True),
        parallel_mode=config.get("model.hybrid.parallel_mode", False)
    )
    ...
    for file_path in files_to_process:
        ...
        try:
            ...
            for page_idx, image in enumerate(images):
                ...
                # Phase 3: Model inference - extract header fields + line items
                ...
                line_count = getattr(processed_result, 'line_items_count', 0)
                logger.info(
